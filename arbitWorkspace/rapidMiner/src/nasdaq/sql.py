...
class sql():
	...
	def fetchInformation(self):
		cursor = self.connection.cursor()
		cursor.execute("SELECT Symbol, Exchange, IPOYear, Sector, Industry, MarketCap FROM NASDAQSymbolInformation")
		...
		for row in rows:	
			symbol = row[0]
			information[symbol]=[]
			information[symbol].append(row[1]) #Exchange
			information[symbol].append(row[2]) #IPOYear
			information[symbol].append(row[3]) #Sector
			information[symbol].append(row[4]) #Industry
			information[symbol].append(row[5]) #MarketCap
		cursor.close()
		
		return information

	def fetchInformationForSymbol(self, symbol):
		cursor = self.connection.cursor()
