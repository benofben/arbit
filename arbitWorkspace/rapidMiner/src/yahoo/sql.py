...
class sql():
	...
	def fetchInformationForSymbol(self, symbol, startDate, endDate):
		cursor = self.connection.cursor()
		...
		for row in rows:
			# Symbol is Index 0
			trainingInformation['Date'].append(row[1].date())			
	...
	def fetchInformation(self, startDate, endDate):
		cursor = self.connection.cursor()
		
		cursor.execute("SELECT Symbol, QuoteDate, Open, High, Low, Close, Volume FROM YahooQuotes WHERE QuoteDate BETWEEN :StartDate AND :EndDate",
			StartDate = startDate,
			EndDate = endDate
		)
		...
		for row in rows:
			trainingInformation['Symbol'].append(row[0])
			trainingInformation['Date'].append(row[1].date())			
			trainingInformation['Open'].append(row[2])
			trainingInformation['High'].append(row[3])
			trainingInformation['Low'].append(row[4])
