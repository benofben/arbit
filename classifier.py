	...
	def createDataPoint(self, day, symbol, predictor):
		...
		if len(self.quotes[symbol]['Open'])>day+1:
                        if self.quotes[symbol]['Low'][day+1]<self.quotes[symbol]['Open'][day+1]*(1-constants.take):
                                delta = constants.take
        		else:
                                delta = (self.quotes[symbol]['Open'][day+1]-self.quotes[symbol]['Close'][day+1])/self.quotes[symbol]['Open'][day+1]
        		dataPoint['Outcome']=(1/constants.take)*delta

                        if dataPoint['Outcome']==1.0:
