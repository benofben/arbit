...
for symbolIndex=1:size(symbols,1)
    ...
    for time=window+1:size(openPrice,1)
        ...
        if(pGoodDay(symbolIndex, time-window)>0.5 && highPrice(time)>openPrice(time)*1.02)
            truePositive=truePositive+1;
        elseif(pGoodDay(symbolIndex, time-window)>0.5 && highPrice(time)<openPrice(time)*1.02)
